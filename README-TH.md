...
> **สถานะ:** v0.4.4 (Alpha) — อยู่บน PyPI (`pip install jarn`) และ npm (`npm install -g jarn-cli` — binary สำเร็จรูป ไม่ต้องมี Python) ปิดช่องว่าง 5 จุดที่ผู้ใช้เจอเทียบกับ harness อื่น (prompt caching, plan-mode handoff, `/commit` + `/review`, background processes, macOS image paste) พร้อมรอบขัดเกลา UX (live in-place streaming, conversation `/rewind`, rich `@`-mentions, in-session `/key`, อนุมัติเร็วขึ้น) ตัว architecture, configuration, permission engine และ terminal REPL พัฒนาเสร็จและผ่านการทดสอบแล้ว การเรียก model จริงต้องมี API key ของตัวเอง ดู [CHANGELOG.md](CHANGELOG.md) และ [SECURITY.md](SECURITY.md)
