...
> current release, **except** the Hermes display-parity close-out block, whose
> `[x]` rows are stacked **draft PRs** (not merged, not a GitHub Release).
> `[ ]` items are scaffolded or documented but not yet shipped.
...
## Hermes display-parity close-out — draft PRs (2026-08-15)
