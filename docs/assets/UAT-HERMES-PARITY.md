...
| Status | Meaning |
|---|---|
| **Covered** | Behavior is asserted in pytest on this branch. The test file is named. |
...
Code is merged on `feat/hermes-parity-closeout` from this PR stack:
...
| T6 | Approval cards persist live prose as a real message then restart the draft; progress `message_id` ≠ draft id; tap deletes the card | **Not run** (live). **Covered** in pytest | `tests/test_telegram_outbox.py` `test_progress_bubble_does_not_destroy_prose_draft`, `test_approval_card_still_destroys_draft_not_progress`, `test_approval_card_persists_live_prose_then_restarts_draft`, `test_settle_approval_card_deletes_the_card`; `tests/test_telegram_bot.py` `test_callback_deletes_card` |
