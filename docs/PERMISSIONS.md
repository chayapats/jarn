...
| User-facing mode (config value) | Reads | Writes | Shell | Network |
|---|---|---|---|---|
| Read only (`plan`) | allow | **deny** | **deny** | **deny** |
| Ask before changes (`ask`, default) | allow | ask | ask | ask |
| Edit workspace (`auto-edit`) | allow | allow *(in-scope)* | ask | allow *(read-only: web + async status)* |
| Full access (`yolo`) | allow | allow | allow | allow |
...
Rules generated by session/always approval are stored in a versioned scoped
envelope. The scope shown in the prompt binds the grant to the action capability,
originating tool, relevant exact target or safe command prefix, and resolved primary
workspace. It cannot become a write because a similarly named shell command was
approved, cannot move between tools, and does not follow a copied project rule into
another workspace. A malformed generated envelope fails closed. Legacy hand-written
string globs remain supported as an intentionally broader explicit policy.
...
> with patterns; it does not fully parse or emulate the shell. Known interpreters
> and indirections (inline evaluators, substitutions, heredocs, aliases/functions,
> dynamic loaders, decoded payloads) force a one-shot confirmation, and visible
> `.jarn/secrets` shell access is blocked. A sufficiently obfuscated program can
> still hide equivalent behavior. For untrusted code use Docker or a required OS
> sandbox; do not rely on Full access plus this net. See [SECURITY.md](../SECURITY.md).
