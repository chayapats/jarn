...
SETTINGS: tuple[Setting, ...] = (
    ...
    _s("context.project_context_tokens", "int", "Behavior", "Project context size",
       "Always-loaded project-guidance excerpt; read the full file on demand."),
    _s("context.memory_tokens", "int", "Behavior", "Memory index size",
       "Shared token budget for global and project memory indices."),
    _s("context.skill_catalog_tokens", "int", "Behavior", "Skill catalog size",
       "Token budget for lazy skill names and descriptions."),
    _s("context.wiki_index_tokens", "int", "Behavior", "Wiki index size",
       "Token budget for the lazy wiki page index."),
    ...
    _s("verify.diagnostics", "enum", "Behavior", "Diagnostics",
       ...
       ("off", "suggest", "auto")),
    _s("verify.diagnostics_max_rounds", "int", "Behavior", "Diagnostics rounds",
       "Max consecutive auto-fix rounds per user turn (loop guard)."),
    _s("verify.diagnostics_ts", "bool", "Behavior", "Diagnostics: tsc",
       "Also type-check with tsc (slow: checks the whole project)."),
    ...
    _s("ui.locale", "enum", "Appearance", "Locale",
       "UI chrome language. auto follows LANG / LC_MESSAGES / LC_ALL (th* → Thai).",
       ("auto", "en", "th")),
    ...
    _s("ui.show_reasoning", "enum", "Appearance", "Thinking display",
       ...
       ("collapsed", "full", "off")),
    _s("ui.thinking_style", "enum", "Appearance", "Thinking style",
       "Thinking-indicator copy: plain (Thinking… / คิด…) or quirky (Cogitating, …).",
       ("plain", "quirky")),
    _s("ui.statusbar", "bool", "Appearance", "Status bar",
       "Show the bottom status bar (model, mode, context)."),
    _s("ui.context_bar", "bool", "Appearance", "Context bar",
       "Show a fill bar for context-window pressure."),
    _s("ui.toolbar_detail", "enum", "Appearance", "Toolbar detail",
       "Status bar density: quiet (model, mode, context) or full (cwd, provider, cost, …).",
       ("quiet", "full")),
