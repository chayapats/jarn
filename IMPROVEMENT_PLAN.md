...
**DoD:**
- [x] `/clear` visually clears scrollback in an interactive check; test asserts the handler path.
...
**DoD:**
- [x] Either multi-turn works with a test, or the flag is removed and CLI help is honest.
- [x] `--json` includes `tool_calls`; errors are structured; exit codes are documented in `--help` and `docs/CONFIGURATION.md` (non-interactive section).
...
**DoD:**
- [x] Test: simulate `_apply_snapshot` failure → redo stack is unchanged (no orphan).
- [x] Test: concurrent snapshot + undo under the lock does not corrupt the stack.
- [x] Dead `label` param resolved.
...
**DoD:**
- [x] Test: after spawning N background jobs and completing them, no parent FDs are leaked (assert via `/proc/self/fd` on Linux or a stub counter cross-platform).
- [x] Test: `list_background` excludes exited processes.
...
**DoD:**
- [x] Micro-benchmark/test: xref build on a synthetic 5k-file fixture completes in linear time (assert runtime ratio vs. a 1k fixture ≈ 5×, not 25×).
- [x] `build_repo_map` no longer forks `git ls-files` per call within the TTL.
...
**DoD:**
- [x] Test: a fake provider emitting cumulative usage on 10 chunks yields the **final** totals once, not 10×.
- [x] Test: concurrent `record` calls don't race (deterministic totals).
...
**DoD:**
- [x] Test: after `/key`, a previously-resolved model re-resolves with the new key (no stale cache).
- [x] Test: `SecretResolutionError` propagates from `fallback_models` instead of being swallowed.
...
**DoD:**
- [x] `/compact` and `/compact status` both work and are tested.
...
**DoD:**
- [x] Test: a 50k-token MEMORY.md is truncated to the configured budget with a notice.
- [x] Test: wiki index injection respects its budget.
...
**DoD:**
- [x] Test: when `.claude` and `.jarn` both define a skill with the same name, doctor and runtime agree on which is active.
...
**DoD:**
- [x] Test: a pyproject without pytest does not suggest `pytest -q`; a pyproject with ruff suggests `ruff check`.
- [x] Test: `test:unit` and `typecheck` scripts are detected.
...
**DoD:**
- [x] Test: a repo with one modified + one untracked file yields a review prompt that includes both.
...
**DoD:**
- [x] A dry-run (act or workflow YAML lint) shows publishes can't start before preflight passes.
- [x] `test_ci` asserts the dependency chain.
...
**DoD:**
- [x] CI runs coverage with a gate; lint covers `scripts`.
- [x] Windows job runs (with documented skips for unsupported features).
...
**DoD:**
- [x] Security job runs on PRs; fails on high-severity vulns / leaked secrets.
...
**DoD:**
- [x] Test fails when docs drift; passes after T-4-9 fixes the numbers.
...
**DoD:**
- [x] PyInstaller version is reproducible from `uv.lock`.
- [x] Release job fails if the binary doesn't smoke.
...
**DoD:**
- [x] Nightly workflow exists, runs on schedule, posts a summary; failure doesn't block PRs.
...
- [x] `/clear`, `--max-turns`/removal, checkpoint undo, background FD, repomap perf, cost dedup, models cache, `/compact`, token budgets, doctor skill precedence, verify, `/review` all fixed and tested.
- [x] Releases cannot publish without passing CI gates; coverage gate, security scan, and binary smoke exist.
- [x] Windows + scripts lint in CI; doc-sync test enforces test counts.
- [x] Full `pytest -q`, `ruff`, `mypy` green.

---


### Phase 2 verification (end of `improve/phase-2-bugs`, after T-2-1…T-2-18)

| Check | Result |
|---|---|
| `uv run ruff check src tests scripts` | **clean** |
| `uv run mypy src/` | **clean** |
| `uv run pytest --collect-only -q` | **1320 tests collected** (+44 vs Phase 1) |
| `uv run pytest -q` (local sandbox) | **1285 passed, 8 skipped, 9 failed, 18 errors** — git-subprocess suites only (+3 new git tests from T-2-3/T-2-12); **no new non-subprocess failures** |
...
- [x] No file exceeds ~750 lines. *(Note: `core.py` is 895 lines after helper extraction — a further pass can move model-rotation / compaction helpers.)*
...
- [x] One slash-command registry drives dispatch + `/help`; no `controller`→`cli` import; doctor render is single-source.
...
### Phase 3 verification (end of `improve/phase-3-refactor`, after T-3-1…T-3-12)
...
| `uv run pytest -q` (local sandbox) | **1291 passed, 8 skipped, 9 failed, 18 errors** — same git-subprocess suites as Phase 2 baseline; **no new non-subprocess failures** |
...
**DoD:**
- [x] `jarn -p "do X" --max-turns 5` runs up to 5 turns; `--resume-session last` continues a prior session; `--json` includes `tool_calls` + structured errors.
- [x] Tests cover multi-turn cap, resume, exit codes, budget stop.
...
- [x] Headless multi-turn + `--resume-session` + structured errors shipped.
- [x] Image paste works on Linux/Windows + format fallbacks + size cap.
